...
int main(int argc, char *argv[]) {
    ...
    printf("---Selects n to remove nth element of the queue:\n");
    unsigned int n = 0;
    scanf("%u", &n);
    if(queue_size(q) < n ) {
    ...
    /* dumping the queue after queue_disscard(q,n); */
    printf("length: %u\n", queue_size(q));
    queue_dump(q, stdout);
    
    /*free memory --> All heap blocks were freed -- no leaks are possible */
    queue_destroy(q);
